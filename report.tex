...
Mahfuz & ReportView with date-range logic, CSV export view, custom template tag library (currency filter, percentage filter, tag\_pills inclusion tag, category colour filter), Chart.js area chart and donut chart in the report template \\[4pt]
Abid   & Entry CRUD views and EntryForm with validation, Category CRUD views, Budget CRUD views and BudgetForm, entry list filtering by type/category/tag/date, pagination, process\_recurring management command \\
\bottomrule
\end{tabular}
...
Mahfuz & Fixed currency symbol in template tags and chart callbacks, fixed category colour hex filter, resolved template tag errors in report page \\[4pt]
